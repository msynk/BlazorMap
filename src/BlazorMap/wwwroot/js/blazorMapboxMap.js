...
function escapeTemplateForMapbox(url) {
  return (url || "").replace("{s}", "a");
}

function getState(mapId) {
  const s = maps.get(mapId);
  if (!s) throw new Error(`BlazorMapboxMap: unknown map id '${mapId}'`);
  return s;
}
...
export async function initMap(mapId, element, dotNetRef, options) {
  ...
  const o = options || {};
  const token = pick(o, "accessToken", "AccessToken") || "";
  mapboxgl.accessToken = token;

  const center = readCenterLngLat(o);
  const zoom = pick(o, "zoom", "Zoom") ?? 13;
  const styleUrl = pick(o, "styleUrl", "StyleUrl") || "mapbox://styles/mapbox/streets-v12";
  ...
  await new Promise((resolve, reject) => {
    ...
    map.once("error", (e) => reject(e.error || new Error("Mapbox map error")));
    setTimeout(() => reject(new Error("Mapbox map load timeout (30s)")), 30000);
  });
...
export function syncMapOptions(mapId, o) {
  ...
  const token = pick(o, "accessToken", "AccessToken");
  if (token !== undefined) mapboxgl.accessToken = token || "";
  ...
  if (styleUrl && styleUrl !== s.lastStyleUrl) {
    ...
    map.once("styledata", () => {
      /* style replaced; vector layers cleared by Mapbox */
...
export function addMarker(mapId, markerId, opts) {
  ...
  if (iconUrl) {
    ...
    marker = new mapboxgl.Marker({ element: el, draggable }).setLngLat([lng, lat]).addTo(s.map);
  } else {
    marker = new mapboxgl.Marker({ draggable }).setLngLat([lng, lat]).addTo(s.map);
  }

  const popupHtml = pick(opts, "popupHtml", "PopupHtml");
  if (popupHtml) {
    marker.setPopup(new mapboxgl.Popup({ offset: 25 }).setHTML(String(popupHtml)));
  }
...
export function addTileOverlay(mapId, opts) {
  ...
  const url = escapeTemplateForMapbox(pick(opts, "urlTemplate", "UrlTemplate"));
